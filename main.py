...
def main():
    if len(sys.argv) > 1:
        ...
        import glob
        # Ищем все .csv и .txt в текущей папке
        candidates = glob.glob("*.csv") + glob.glob("*.txt")
        # Исключаем возможные файлы не с данными (на всякий случай можно оставить все)
        if len(candidates) == 0:
            print("Ошибка: в текущей папке нет файлов .csv или .txt.")
            print("Укажите имя файла: python main.py ваш_файл")
            sys.exit(1)
        elif len(candidates) == 1:
            filepath = candidates[0]
            print(f"Найден файл: {filepath}")
        else:
            print("Найдено несколько файлов .csv/.txt:")
            for i, f in enumerate(candidates):
                print(f"  {i+1}. {f}")
            print("Укажите номер файла или запустите с аргументом: python main.py имя_файла")
            choice = input("> ")
            try:
                idx = int(choice) - 1
